...
try:
    import Spec_Module_withSiemens as sp
except:
    from mrs import Spec_Module_withSiemens as sp
import csv
...
# ------Main Window Geometry-----------
        ...
        # self.pw1.setXRange(min(self.specoblist[self.curobject].fake_ppms), max(self.specoblist[self.curobject].fake_ppms))
        # self.pw1.setYRange(min(np.real(self.specoblist[self.curobject].OriginalSpectrum)), max(np.real(self.specoblist[self.curobject].OriginalSpectrum)))
        ...
        # self.pw2.setXRange(min(self.specoblist[self.curobject].fake_ppms), max(self.specoblist[self.curobject].fake_ppms))
        # self.pw2.setYRange(min(np.real(self.specoblist[self.curobject].current_frame)), max(np.real(self.specoblist[self.curobject].current_frame)))
...
#        print(f'App size {w}x{h} px')
    ...
    def check_btnopen(self):
        ...
        if "2) " in self.btnopen.text():
            self.btnopen.move(int(200*self.wf*self.scale), int((10+dy)*self.hf*self.scale))
            framenum = self.specoblist[self.curobject].curframe + 1
            # totalframes = int(self.specoblist[self.curobject].Frames / 2)  # for siemens with no Wref
            totalframes = int(self.specoblist[self.curobject].Frames)    
            self.btnopen.setText(f"2) Check and adjust spectra if needed: {str(framenum)}/{str(totalframes)}")
            self.btnopen.setStyleSheet("QPushButton {color: #f2c885;}") 

        # if int(self.specoblist[self.curobject].curframe + 1) == int(self.specoblist[self.curobject].Frames / 2):
        if int(self.specoblist[self.curobject].curframe + 1) == int(self.specoblist[self.curobject].Frames ): #for siemens with no Wref
            self.btnreport.show()
    ...
    def plotfit(self):
        try:
            ...
            # self.pw1.setXRange(min(PPM_arr), max(PPM_arr))
            # self.pw1.setYRange(min(Data_arr), max(Data_arr))
