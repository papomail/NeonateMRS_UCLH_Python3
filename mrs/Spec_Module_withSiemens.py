...
class SpecObject():
    ...
    def __init__(self, filename, dirpass):
        ...
        try:
            self.ds = dcm.read_file(self.filename)
                # self.ds, header = rds.read_dicom(self.filename)
                # data = dicom[TAG_SPECTROSCOPY_DATA].value
                # data = self.ds[0x7fe1, 0x1010].value
        ...
        if [0x5600,0x0020] in self.ds:
            ...
            if self.ds[0x0029, 0x1008].value == 'SPEC NUM 4': 
                ...
                self.SeriesTime = self.ds.SeriesTime
                data = self.ds[0x7fe1, 0x1010].value
                self.SpecData = struct.unpack("<%df" % (len(data) / 4), data)
                # self.SpecData = self.ds[0x5600,0x0020].value
                self.Datapoints = int(info['[CSA Image Header Info]']['DataPointColumns'])
                self.SpectralWidth = float(info['[CSA Image Header Info]']['PixelBandwidth'])
                # self.TransmitterFrequency = self.ds.TransmitterFrequency
                # self.TransmitterFrequency = info['[CSA Image Header Info]']['PixelBandwidth']
                #self.FieldStrength = self.ds[0x0018,0x0087].value
                self.FieldStrength = info['[CSA Image Header Info]']['MagneticFieldStrength']
                self.PatID = self.ds.PatientID
                self.AcquisitionDateTime = self.ds.AcquisitionDate
                self.ProtocolName =info['[CSA Image Header Info]']['SequenceName']
                # self.ProtocolName = self.ds.ProtocolName            
                self.Frames = int(info['[CSA Image Header Info]']['NumberOfFrames'])
    ...
    def autophase(self):
        ...
        for cnt in range(0, frames):
            ...
            while got_peaks < 1:
                found_peaks = 0
                peak_pos = []
                for cntr in range(self.plim_l,self.plim_r):
    ...
    def writeTarquin(self, outpath):
        ...
        if self.isspec == 3:       
            # self.SpecData = struct.unpack("<%df" % (len(data) / 4), data) 
            packed_data = struct.pack("<%df" % len(Spec_temp), *Spec_temp)
            self.ds[0x7fe1, 0x1010].value = packed_data   
        elif self.isspec == 2:   
...
class PDF(FPDF):
    ...
    def header(self):
        ...
        # Your paragraph
        text = '******************************************************************************************************************\nSpectrum acquired at 1.5T. Take care when comparing with historical data acquired at 3T.\nNumerical value for indication only and not direct comparison to previous 3T data with outcomes.\n*****************************************************************************************************************************'
        self.cell(10)
        self.multi_cell(175, 5, text, 0, 'C')  # height, text, border, alignment. Alignment won't work as expected but anyway we have aligned it with set_x
        self.set_font('Arial', '', 10)
